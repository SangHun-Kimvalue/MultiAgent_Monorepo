...
| `approval_gate` 미배선(T10-B3a) | 소비자 0건 + `approval_challenges`/`approval_audit` 스키마가 모든 DB에 무조건 생성(2026-07-28 실측). T10 게이트③에서 B3b 미선택 시 스키마 조건부화 + 미배선 명시로 정리. 판단 재료는 §10 T10 행 |
| 테스트 비대 상환 | tests 11,214줄 vs prod 8,019줄(2026-07-28 실측). "지우지 않고 추가 기준을 낮춰 갚는다" 유예 판단 유지 + 집행 규칙: 신규 테스트는 잠그는 결함·게이트를 주석으로 명시, 슬라이스의 테스트 증가분이 제품 증가분을 넘으면 리뷰에서 사유 요구 |
