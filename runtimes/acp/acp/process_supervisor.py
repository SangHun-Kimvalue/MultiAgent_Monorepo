...
⚠ **이 파일은 `runtimes/ztr/src/engine/process_supervisor.py` 와 의도적으로 같은 내용이다.**
ADR AD-3 이 acp → ztr import 를 금지하고 두 런타임의 venv 가 분리돼 있어 코드를 공유할 수
없다. 중립 공용 패키지로의 통합은 **미검토**이며 별도 슬라이스다(계획 §5).
한쪽을 고치면 **반드시 다른 쪽도 고친다** — 이 슬라이스는 "한 벌만 고쳐진" 결함에서 출발했다.
