...
## prepare-session-compaction (양 surface 공용)

`prepare-session-compaction` 스킬은 `.claude/skills/*`와 `.codex/skills/*` 양쪽에 동일 `SKILL.md`로
설치되는 공용 어댑터다. 사용자가 `압축 준비해`처럼 요청하면 압축 전에 durable 상태를 Sync-Out(HANDOFF/
lessons/ROADMAP + 호스트가 허용하는 메모리)하고, 압축 후 새 컨텍스트가 SoT를 다시 읽게 하는 재개
프롬프트를 채팅 fenced block + 고정 artifact `.agent-workflow/compaction/resume-prompt.md`로 만든다.
provider별 동작 사본은 두지 않는다(캐논이 SSoT, 스킬은 얇은 실행 어댑터).

plugin으로 활성화된 Claude/Codex surface에는 공용 `SessionStart(compact)` hook
(`plugins/agent-workflow/hooks/hooks.json` + `scripts/emit_compaction_boot_context.py`)이 함께 배포돼,
압축 직후 다음 model request에 read-only live git facts와 위 artifact 경로/excerpt를 developer context로
주입한다. hook은 새 user message나 zero-turn 실행을 만들지 않는다. standalone `.claude/skills`/`.codex/skills`
복사본만 설치된 환경에는 hook이 없으므로 수동 `압축 준비해`와 artifact 경로 안내를 유지한다.
